...
function sumOfCodes(str) {
  ...
  return str
    ...
    .reduce((accum, code) => accum + code, 0);
}
...
function countVowels(str) {
  return str
    .toLowerCase()
    .split('')
    .filter((char) => 'aeiouy'.includes(char)).length;
}
...
function isPalindrome(str) {
  const filtered = str.toLowerCase().replace(/[^a-zA-Z]/g, '');
...
function invertCase(str) {
  return str
    .split('')
    .map((char) =>
      char.charCodeAt(0) > 91 ? char.toUpperCase() : char.toLowerCase()
    )
    .join('');
}
...
function extractNameFromTemplate(value) {
  const arr = value.replace(/[^a-zA-Z\s]/g, '').split(' ');
  return `${arr[1]} ${arr[2]}`;
}
